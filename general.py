

import os


#Each website you crawl is a seperate project
def create_project_dir(directory):
    if not os.path.exists(directory):
        print('Creating new project')
        os.makedirs(directory)
    
#create queue and crawled files(if not created)
def create_data_files(project_name, base_url):
    queue = project_name + '/queue.txt'
    crawled = project_name + '/crawled.txt'
    scores = project_name + '/scores.txt'
    if not os.path.isfile(queue):
        write_file(queue, base_url)
    if not os.path.isfile(crawled):
        write_file(crawled, '')
    if not os.path.isfile(scores):
        write_file(scores, '')
    
#create a new file
def write_file(path, data):
    f = open(path, 'w')
    f.write(data)
    f.close()
    
#add data into files
def append_to_file(path, data):
    with open(path, 'a') as file:
        try:
            file.write(data + '\n')
        except:
            #print('cannot update link', data)
            pass
#delete the contents of the file
def delete_file_contents(path):
    with open(path,'w'):
        pass

#Read a file and convert each line to set lines
def file_to_set(file_name):
    results=set()
    with open(file_name,'rt') as f:
        for line in f:
            results.add(line.replace('\n',''))
    return results

#Iterate through a set, each item will be a new line in the file
def set_to_file(links, file):
    delete_file_contents(file)
    for link in sorted(links):
        append_to_file(file,link)
        
def dictionary_to_file(dictionary,file):
    delete_file_contents(file)
    sorted_by_value = sorted(dictionary.items(), key=lambda kv: kv[1])
    for x in sorted_by_value[::-1]:
        append_to_file(file,str(x))
        





















